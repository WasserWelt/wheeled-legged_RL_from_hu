...
For the `gpu_isaac` and `3090_wyw_local` server workflows, use
`scripts/cloud/fdu_flat_train_pipeline.sh`. It starts detached Flat training,
records checkpoint-aligned training videos, waits for completion, runs a
headless Play acceptance recording, preserves the MP4 path, and records the
acceptance result. Server-specific setup and artifact paths are documented in
`docs/cloud_gpu_isaac_workflow.md` and `docs/lab_3090_server_workflow.md`.
