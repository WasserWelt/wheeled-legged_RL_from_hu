...
3. **L0 目标**：机械竖直范围约 `[0.09495, 0.34149] m`，但该范围不等于数值稳定训练范围。
   当前 WYW 的目标投影范围 `[0.23, 0.31] m` 在 500 Hz 下稳定；Fudan 的 `L_tuck=0.16 m` 或冲击
   承重仍可能使实测腿长进入 `<=0.14 m` 边界，必须查看 §3.1 日志。若越界频繁，优先比较已验证的
   800 Hz 档或重构闭环约束。不要把几何/限位可达直接当作训练安全范围。
