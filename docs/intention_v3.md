...
四个腿动作直接按 `q_target=q_default+0.5*action` 发送给实体驱动杆，与 Fudan 的直接关节目标语义一致；
不再按 `L0/theta0` 做安全投影、裁剪或逆解。解析等效腿状态只供 reward、几何诊断和稳定边界监控使用。
因此高度命令 `[0.15,0.30] m` 不再被旧 `[0.23,0.31] m` 目标工作区截断，但闭链可达性完全交给物理
约束、实体关节限位和力矩上限处理。新 PPO 验收必须查看 `base_height` 分桶回报、实测 root height、
腿力矩饱和率和 `FDU_L0Boundary` 进入次数，且不得暗中 clamp observation 里的 command。
...
400/200 Hz 下分别约为 `12.0/24.5 mm`；800 Hz 是已验证的低腿长缓解档，当前 500 Hz 正式配置则必须
通过 §3.1 的实测边界监控追踪动态越界。
...
`500*32` 已高于 `800*16`，因此不能用堆迭代代替减小时间步。当前正式选择统一使用 500 Hz、
`16/6`、`decimation=5`，维持 100 Hz 策略周期；`0.14 m` 是监控边界而非动作限位或终止条件。
800 Hz、`16/6` 作为确认过的故障缓解档保留，只有训练日志证明实际频繁进入边界后再评估启用。
...
2026-08-30 将四个腿驱动硬上限改为 `40 N·m` 后，以完全相同的 500 Hz、`16/6`、无自碰撞、竖直导向
条件重新录像。`L0=0.285 m` 工况的峰值驱动力矩为 `34.00 N·m`、闭链残差 `0.756 mm`、最终
`L0=0.2175 m`；`L0=0.16 m` 工况分别为 `34.09 N·m`、`1.377 mm`、`0.1282 m`。两者都未达到
40 N·m 硬限，且与 30 N·m 基线的最终腿长近似，说明提高上限主要增加冲击瞬间裕量，并不会消除短腿
低缓冲工况的落地压缩和跟踪误差。用户确认接受该冲击误差，因此生产配置保持 Flat/Rough
`Kp=20,Kd=1`、Jump `Kp=6,Kd=0.5`、硬上限 `40 N·m`，不因该跌落实验继续增硬。
一次隔离诊断 `Kp=30,Kd=2` 的确减小了常规腿长的稳态偏差，但冲击力矩达到 40 N·m；它只保存在
`fdu_validation/drop/` 作 A/B 资料，不进入训练配置。两组正式 40 N·m 录像位于
`fdu_validation/video/fdu_drop_guided_500hz_l016_h040_40nm.mp4` 和
`fdu_drop_guided_500hz_l0285_h045_40nm.mp4`。

短腿录像后半段仍有持续抖动，不能归类为正常的落地瞬态。500 Hz、`16/6` 下，目标 `L0=0.16 m`
承重后实际落到约 `0.128 m`，进入静态扫描已识别的 `0.12--0.13 m` 数值极限环区，末 0.5 s 左右腿
峰峰值为 `4.366/4.292 mm`。完全相同的 40 N·m/PD/跌落条件只把频率提高到 800 Hz、`16/6` 后，
末段峰峰值降为 `0.282/0.108 mm`，约降低 15--40 倍；因此主因是闭链约束的离散求解稳定性，而不是
连杆自碰撞、力矩限幅或应被接受的物理缓冲振荡。800 Hz 对照录像为
`fdu_validation/video/fdu_drop_guided_800hz_l016_h040_40nm.mp4`。
500 Hz、`32/6` 也可把末段抖动降至 `0.076/0.098 mm`，但粗略约束求解工作量 `500*32` 高于
800 Hz、`16/6` 的 `800*16`，且后者覆盖更短构型。尽管如此，当前按已确认方案正式采用
500 Hz、`16/6`；通过下述运行时监控确定训练是否真实进入该边界，而不根据单个严苛跌落样例预先提高
全部训练成本。提高频率后记录到的单步峰值接触力不能与 500 Hz 直接比较；时间步
不同会改变冲量在采样点上的峰值表示，应另用接触冲量或力的时间积分比较冲击强弱。

## 3. 时间和缓冲区契约

正式配置固定为物理 `dt=0.002 s`（500 Hz）、`16/6` position/velocity iterations、
`decimation=5`，策略周期 `step_dt=0.01 s`（100 Hz），episode 20 s。Flat、Rough、Jump 和 Play 共用
这一物理契约；`render_interval=5`。提高物理频率不改变 reward 积分、命令周期、终止持续时间和动作历史
的 100 Hz 语义。

### 3.1 500 Hz 稳定边界运行时监控

环境在每个 500 Hz 物理子步直接用 PhysX body pose 测量左右 hip-to-wheel 平面距离，而不是用驱动角解析
重建的理想腿长；越界状态和最小值在 GPU 上锁存，每个 100 Hz 策略步最多同步/输出一次，避免监控造成
逐物理步 CPU 同步开销。任一侧 `L0 <= 0.14 m` 只触发诊断，不裁剪动作、不改奖励、也不 reset：

- 终端与 Isaac 日志使用固定检索前缀 `[WYW:FDU:L0-STABILITY-BOUNDARY]`，首次进入立即发出 WARNING；
  持续越界时每 1000 个策略步重复一次，恢复时记录 `RECOVERED`。
- WARNING 至少包含策略 step、当前越界环境数/总数、前 16 个环境 ID、该策略周期各环境最小实测 `L0`、当前批次和历史
  最小值、累计进入事件数，并提示按固定前缀搜索。
- runner/TensorBoard 的 `Diagnostics/FDU_L0Boundary/*` 持续记录当前越界数量/比例、当前/历史最小 `L0`、
  累计越界物理样本和进入事件；reset 时另写 `Episode/FDU_L0Boundary/*`，包括受影响环境比例、平均越界
  物理样本数、进入次数和 episode 最小腿长。

该监控必须保持为诊断设施，不能静默变成训练 shaping。若日志出现大量或长时间越界，再结合相应 env ID、
命令和地形复现，并优先比较 800 Hz、`16/6`；不能仅靠增大 PD 或力矩掩盖闭链数值极限环。
...
| Pure-tensor golden tests | 20 项，Isaac Lab Python/CPU | 通过：mapping、25/141 分段、actor-noisy/critic-clean、历史、Plane/Jump 公式、逐项 clip、DR、contact filter、termination、课程 |
| Flat 直接四杆动作 smoke | 1 env、CPU、500 Hz/16/6、reset + 三次 step | 通过：不经 `L0/theta0` 投影，obs/reward 有限；合成短腿样本触发边界日志 |
...
由 reset 重新采样，因此不会出现独立的 step-2000 周期重采样。表中的 64-env PPO 使用已删除的
`L0/theta0` 投影动作语义，不能继续作为当前直接四杆目标配置的验收证据；三任务都必须重跑 GPU smoke。
旧 Jump 结果长期、广泛进入 `L0<=0.14 m` 边界，仍是明确的历史物理失败证据。
...
三次运行使用物理 `500 Hz`、`16/6` solver、`decimation=5`、腿驱动硬上限 `40 N·m` 和高度命令范围
`[0.15,0.30] m`，但仍包含现已删除的 `[0.23,0.31] m` 动作投影。它们只能证明旧版本 Flat/Rough
优化器数值有限以及旧 Jump 的无弹簧物理失败，不能证明当前直接四杆动作路径。对应自然长程语义报告
位于 `docs/fdu_validation/training/`，同样保留为历史对照。

Jump 固定目标报告为 `fdu_validation/jump/fdu_jump_calibration_500hz_40nm.json`。当前证据支持的因果链是：
Jump 沿用 Fudan `Kp=6,Kd=0.5`，但当前 FDU 配置尚无原版 spatial tendon 气弹簧支撑；旧版即使将
目标投影限制在 `L0>=0.23 m`，实际承重腿长仍压到约机械下限。当前已移除该投影，后续必须在以下两条
路线中明确选择并单独验收：
...
3. **L0 目标**：机械竖直范围约 `[0.09495, 0.34149] m`，但该范围不等于数值稳定训练范围。
   当前四驱动杆目标不再投影；Fudan 的 `L_tuck=0.16 m`、直接关节动作或冲击承重都可能使实测腿长
   进入 `<=0.14 m` 边界，必须查看 §3.1 日志。若越界频繁，优先比较已验证的 800 Hz 档或重构闭环
   约束。不要把几何/限位可达直接当作训练安全范围。
