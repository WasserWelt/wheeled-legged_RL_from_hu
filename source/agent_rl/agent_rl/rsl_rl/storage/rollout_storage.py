...
class RolloutStorageDreamWaq:
    ...
    def __init__(
        ...
        assert isinstance(observations_shape, dict)
        # store inputs
        self.device = device
        ...
        # Core
        self.observations = {k: torch.zeros(num_transitions_per_env, num_envs, v, device=self.device) for k, v in
                             observations_shape.items()}
        self.next_observations = {k: torch.zeros(num_transitions_per_env, num_envs, v, device=self.device) for k, v in
                                  observations_shape.items()}
    ...
    def add_transitions(self, transition: Transition):
        ...
        for k in transition.observations:
            ...
            if transition.next_observations is not None and k in transition.next_observations:
                self.next_observations[k][self.step].copy_(transition.next_observations[k])
            else:
                self.next_observations[k][self.step].copy_(transition.observations[k])
            if torch.any(torch.isnan(transition.observations[k])):
                print(k)
                print('WARNING: observations loss')  
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        next_observations = {k: v.flatten(0, 1) for k, v in self.next_observations.items()}
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                # -- Core
                observations_batch = {k: v[batch_idx] for k, v in observations.items()}
                next_observations_batch = {k: v[batch_idx] for k, v in next_observations.items()}
                ...
                yield observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, rewards_batch, \
                       old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (None, None), None, \
                       next_observations_batch, dones_batch
                


class RolloutStorageHIM:
    class Transition:
        def __init__(self):
            self.observations = None
            self.actions = None
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                old_mu_batch = old_mu[batch_idx]
                old_sigma_batch = old_sigma[batch_idx]
                yield observations_batch, next_critic_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, \
                       old_actions_log_prob_batch, old_mu_batch, old_sigma_batch

from agent_rl.rsl_rl.utils.utils import split_and_pad_trajectories
