...
def notify_env_training_progress(env, iteration: int, mean_total_reward: float | None = None):
    """通知 env 当前训练进度，用于 use_init_time_random 等条件触发。
    mean_total_reward: episode 总 reward 的滑动均值，由 runner 从 rewbuffer 计算。
    """
    unwrapped = getattr(env, "unwrapped", env)
    if hasattr(unwrapped, "set_training_progress"):
        unwrapped.set_training_progress(iteration=iteration, mean_total_reward=mean_total_reward)
import numpy as np
