...
class HIMEstimator(nn.Module):
    def __init__(self,
        ...
        enc_layers += [nn.Linear(enc_input_dim, enc_hidden_dims[-1] + num_estimate)]
    ...
    def update(self, obs_history, next_critic_obs, lr=None):
        ...
        vel = next_critic_obs[:, self.num_one_step_obs:self.num_one_step_obs+self.num_estimate].detach()
        next_obs = next_critic_obs.detach()[:, self.num_estimate:self.num_one_step_obs+self.num_estimate]

        z_s = self.encoder(obs_history)
        z_t = self.target(next_obs)
        pred_vel, z_s = z_s[..., :self.num_estimate], z_s[..., self.num_estimate:]
